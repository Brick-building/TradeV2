...
import {
  Activity, Zap, TrendingUp, TrendingDown, Settings,
  ChevronDown, ChevronUp, Plus, RefreshCw, Circle,
...
function Ticker({ label, value, sub, color, mono }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 10, letterSpacing: '0.15em', color: 'var(--text3)', fontFamily: 'var(--cond)', textTransform: 'uppercase' }}>{label}</span>
      <span style={{
        fontSize: 22,
        fontFamily: mono ? 'var(--mono)' : 'var(--cond)',
...
function StrategyCard({ strategy, onUpdate }) {
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      await api.updateStrategy(strategy.id, { config: parsed })
      setEditing(false)
      onUpdate()
    } catch (e) { setErr(e.message) }
    setSaving(false)
  ...
  const statusColor = strategy.enabled
    ? (strategy.has_class ? 'var(--green)' : 'var(--yellow)')
    : 'var(--text3)'

  return (
    <div style={{
      border: `1px solid ${strategy.enabled ? 'var(--border-bright)' : 'var(--border)'}`,
      background: strategy.enabled ? 'var(--bg2)' : 'var(--bg1)',
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: 'var(--cond)', fontWeight: 700, fontSize: 15, letterSpacing: '0.05em' }}>
            {strategy.name}
          </div>
          {strategy.description && (
            <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>{strategy.description}</div>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {strategy.poll_interval_seconds && (
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text3)' }}>
              /{strategy.poll_interval_seconds}s
            </span>
          )}
          {!strategy.has_class && (
            <span style={{ fontSize: 10, color: 'var(--yellow)', fontFamily: 'var(--mono)' }}>NO CLASS</span>
          )}
          <button
            ...
            style={{
              padding: '3px 10px',
              fontSize: 10, fontFamily: 'var(--mono)', letterSpacing: '0.1em',
              background: strategy.enabled ? 'var(--green-bg)' : 'var(--bg3)',
              color: statusColor,
        ...
        <div style={{ padding: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <button
              ...
              style={{ fontSize: 10, color: 'var(--blue)', background: 'none', padding: '2px 6px' }}
            >
              {editing ? 'CANCEL' : 'EDIT'}
            </button>
          </div>
          {editing ? (
            ...
            <pre style={{
              ...
              border: '1px solid var(--border)', overflowX: 'auto', lineHeight: 1.7,
            }}>
...
function NewStrategyForm({ onCreated }) {
  ...
  const submit = async () => {
    ...
    setErr(null)
    try {
      await api.createStrategy({ ...form, config: JSON.parse(form.config) })
      setShow(false)
      onCreated()
    } catch (e) { setErr(e.message) }
    setSaving(false)
  ...
  if (!show) return (
    <button
      ...
      style={{
        ...
        border: '1px dashed var(--blue)55', width: '100%',
  ...
  return (
    <div style={{ border: '1px solid var(--blue)44', background: 'var(--bg2)', padding: 16 }}>
      <div style={{ fontSize: 12, color: 'var(--blue)', fontFamily: 'var(--mono)', marginBottom: 12, letterSpacing: '0.1em' }}>
        NEW STRATEGY
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <input placeholder="strategy_name (must match Python class name)" value={form.name}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={submit} style={{
            padding: '6px 14px', fontSize: 11, background: 'var(--blue-bg)',
            color: 'var(--blue)', border: '1px solid var(--blue)44',
          }}>
            {saving ? 'CREATING...' : 'CREATE'}
          </button>
          <button onClick={() => setShow(false)} style={{
            padding: '6px 14px', fontSize: 11, background: 'none',
...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [p, h, d, s, st] = await Promise.all([
  ...
  const recentBuys = decisions.filter(d => d.action === 'buy').length
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <header style={{
        ...
        <div style={{
          fontFamily: 'var(--cond)', fontWeight: 900, fontSize: 20,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text3)' }}>
          ...
          {lastUpdate && <span style={{ color: 'var(--text3)' }}>· {fmtTime(lastUpdate.toISOString())}</span>}
          <button
            onClick={refresh}
            style={{ background: 'none', color: 'var(--text3)', padding: '4px', marginLeft: 4 }}
          >
            <RefreshCw size={12} />
          </button>
        </div>
          ...
          <>
            ...
            {tab === 'dashboard' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {/* Chart */}
                <div style={{
                  border: '1px solid var(--border)', background: 'var(--bg1)', padding: '16px 16px 8px',
                }}>
                  <div style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'var(--mono)', letterSpacing: '0.12em', marginBottom: 12 }}>
                    PORTFOLIO VALUE / 2H
                  </div>
                  {history.length > 1 ? (
                    <ResponsiveContainer width="100%" height={180}>
                      <AreaChart data={history} margin={{ top: 4, right: 0, bottom: 0, left: 0 }}>
                        ...
                        <XAxis
                          ...
                          tick={{ fontFamily: 'var(--mono)', fontSize: 10, fill: 'var(--text3)' }}
                    ...
                    <div style={{ height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text3)', fontFamily: 'var(--mono)', fontSize: 11 }}>
                      NO SNAPSHOT DATA YET — portfolio is snapshotted every 60s
                    </div>
                  )}
                </div>
                ...
                {positions.length > 0 && (
                  <div style={{ border: '1px solid var(--border)', background: 'var(--bg1)' }}>
                    <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', fontSize: 10, color: 'var(--text3)', fontFamily: 'var(--mono)', letterSpacing: '0.12em' }}>
                      OPEN POSITIONS
                    </div>
                ...
                <div style={{ border: '1px solid var(--border)', background: 'var(--bg1)' }}>
                  ...
                  {decisions.slice(0, 8).map(d => (
                    <div key={d.id} style={{
                      ...
                      {d.contract_price && (
                        <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text2)' }}>{(d.contract_price * 100).toFixed(0)}¢</span>
                      )}
                      {d.time_remaining_seconds != null && (
                        <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text3)' }}>{d.time_remaining_seconds}s</span>
                      )}
            ...
            {tab === 'strategies' && (
              <div style={{ maxWidth: 720, display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {strategies.map(s => (
                  <StrategyCard key={s.id} strategy={s} onUpdate={refresh} />
                ))}
                <NewStrategyForm onCreated={refresh} />
                <div style={{
                  marginTop: 8, padding: 14,
                  border: '1px solid var(--border)', background: 'var(--bg1)',
                  fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)', lineHeight: 1.8,
                }}>
            ...
            {tab === 'log' && (
              <div>
                <div style={{ marginBottom: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'var(--mono)', letterSpacing: '0.12em' }}>
                  ...
                  {['buy', 'skip', 'error'].map(a => (
                    <span key={a} style={{
                      ...
                      color: a === 'buy' ? 'var(--green)' : a === 'error' ? 'var(--red)' : 'var(--text3)',
                    }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      ...
                      <tr style={{ borderBottom: '1px solid var(--border)' }}>
                        {['TIME', 'ACTION', 'SIDE', 'TICKER', 'PRICE', 'SECS LEFT', 'CONTRACTS', 'SIZE', 'REASON'].map(h => (
                          <th key={h} style={{
                            ...
                            color: 'var(--text3)', fontFamily: 'var(--mono)', letterSpacing: '0.12em',
                            whiteSpace: 'nowrap',
                          }}>{h}</th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {decisions.map((d, i) => (
                        <tr key={d.id} style={{
                          borderBottom: '1px solid var(--border)22',
                          background: i % 2 === 0 ? 'transparent' : 'var(--bg2)11',
                          ...
                          <td style={{ padding: '7px 12px', fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text2)' }}>
                            {d.contract_price ? `${(d.contract_price * 100).toFixed(0)}¢` : '—'}
                          </td>
                          <td style={{ padding: '7px 12px', fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text2)' }}>
                            {d.time_remaining_seconds ?? '—'}
                          </td>
